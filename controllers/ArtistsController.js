...
exports.createNew = async (req, res) => {
    ...
    if (!req.body.dob) {
        return res.status(400).send({ error: "Required parameter 'Date Of Birth' is missing" })
    }
    if (!req.body.gender) {
        return res.status(400).send({ error: "Required parameter 'Gender' is missing" })
    }
...
exports.getAll = async (req, res) => {
    const result = await artists.findAll({ attributes: ["id", "name", "dob", "gender"] })
    res.json(result)
}
